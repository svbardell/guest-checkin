...
// tag color system based on event time windows (PST)
const TAG_COLORS = [
...
function getCurrentTagColor() {
 ...
 return TAG_COLORS.find((s) => h >= s.startHour && h < s.endHour) || null;
}

function TagBanner({ currentSlot }) {
 const [active, setActive] = useState(getCurrentTagColor);

 useEffect(() => {
 const id = setInterval(() => setActive(getCurrentTagColor()), 60000);
 return () => clearInterval(id);
 }, []);

 const display = currentSlot
 ? TAG_COLORS.find((s) => s.slot === currentSlot) || active
 : active;
 ...
 <div style={{ fontSize: 13, color: "#6b7280", marginTop: 2 }}>Event hours: 3:00–7:00 PM · Select a timeslot to see the tag color</div>
 </div>
 </div>
 );
 }
 ...
 Issue a {display.color.toUpperCase()} tag
 </div>
...
function GuestRow({ guest, currentSlot, onCheckin, onUndo }) {
 ...
 const tag = TAG_COLORS.find((s) => slotsMatch(s.slot, guest.slot));
 ...
 {tag && (
 <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, color: tag.hex, background: tag.bg, border: `1px solid ${tag.border}`, padding: "1px 7px", borderRadius: 20 }}>
 {tag.emoji} {tag.color} tag
 </span>
 )}
 </div>
 </div>
...
export default function GuestCheckIn() {
 ...
 const exportCSV = () => {
 const headers = ["First Name", "Last Name", "MLO", "Guest Name", "Assigned Slot", "tag color", "Checked In", "Check-In Time (PST)", "Walk-in"];
 const rows = guests.map((g) => {
 const tag = TAG_COLORS.find((s) => slotsMatch(s.slot, g.slot));
 return [g.first, g.last, g.mlo, g.guestName, g.slot, tag ? tag.color : "", g.checkedIn ? "Yes" : "No", g.checkedInTime || "", g.isWalkIn ? "Yes" : "No"];
 });
 ...
 <TagBanner currentSlot={currentSlot} />
 ...
 <div style={{ fontSize: 15, color: "var(--text)", fontWeight: 500, marginBottom: 6 }}>Drop your CSV here</div>
 ...
 {TIMESLOTS.map((s) => {
 const sc = TAG_COLORS.find((c) => c.slot === s.value);
 return (
 <button
 key={s.value}
