...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const validSlots = ["3:00","4:00","5:00","6:00"]; if (!validSlots.some(s => slot.includes(s))) continue;
    ...
    <div style={{ background: rowBg, border: `1px solid ${rowBorder}`, borderRadius: 10, padding: "12px 16px", transition: "all 0.15s ease" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          ...
          <button onClick={() => !guest.checkedIn && onCheckin(guest.id)} style={{ fontSize: 13, fontWeight: 500, padding: "6px 16px", borderRadius: 8, border: guest.checkedIn ? "1px solid var(--success-border)" : "1px solid var(--accent)", background: guest.checkedIn ? "transparent" : "var(--accent)", color: guest.checkedIn ? "var(--success-text)" : "#fff", cursor: guest.checkedIn ? "default" : "pointer", transition: "all 0.1s" }}>
  ...
  return (
    <div style={{ "--text": "#0f0f0f", "--muted": "#6b7280", "--surface": "#ffffff", "--bg": "#f5f4f1", "--border": "#e5e7eb", "--accent": "#1a56db", "--accent-soft": "#eff6ff", "--success-bg": "#f0fdf4", "--success-border": "#86efac", "--success-text": "#15803d", "--warn-bg": "#fffbeb", "--warn-border": "#fcd34d", "--warn-text": "#92400e", fontFamily: "'Inter', system-ui, sans-serif", background: "var(--bg)", minHeight: "100vh", padding: "32px 20px" }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        {!loaded && (
          <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); handleFile(e.dataTransfer.files[0]); }} style={{ border: "2px dashed var(--border)", borderRadius: 14, padding: "40px 24px", textAlign: "center", marginBottom: 24, background: "var(--surface)" }}>
        ...
        {loaded && (
          <input type="text" placeholder="Search by member name, MLO, or guest name..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: "100%", padding: "10px 14px", fontSize: 14, border: "1px solid var(--border)", borderRadius: 10, background: "var(--surface)", color: "var(--text)", marginBottom: 10, outline: "none", boxSizing: "border-box" }} />
        )}
        {loaded && currentSlot && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {loaded ? (filtered.length > 0 ? filtered.map((g) => <GuestRow key={g.id} guest={g} currentSlot={currentSlot} onCheckin={checkin} onUndo={undo} />) : <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontSize: 14 }}>No guests match "{search}"</div>) : <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontSize: 14 }}>Upload a CSV to get started.</div>}
        </div>
      </div>
    </div>
  );
