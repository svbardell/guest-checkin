  ...
  if (!display) {
        return (
                <div style={{ background: "#f3f4f6", border: "1px solid #e5e7eb", borderRadius: 12, padding: "14px 20px", marginBottom: 24, display: "flex", alignItems: "center", gap: 12 }}>
                          <span style={{ fontSize: 22 }}>🏷️</span>span>
                          <div>
                                    <div style={{ fontWeight: 600, fontSize: 15, color: "#374151" }}>No active check-in window</div>div>
                                    <div style={{ fontSize: 13, color: "#6b7280", marginTop: 2 }}>Event hours: 3:00–7:00 PM · Select a timeslot to see the sticker color</div>div>
                          </div>div>
                </div>div>
              );
  }
          ...
          <div style={{ background: display.bg, border: `2px solid ${display.border}`, borderRadius: 12, padding: "16px 20px", marginBottom: 24, display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                        ...
                        <div style={{ fontSize: 13, color: "#6b7280", marginTop: 3 }}>
                                  Current window: <strong style={{ color: "#374151" }}>{display.slot}</strong>strong>
                          {!currentSlot && <span style={{ marginLeft: 8, fontSize: 12, background: display.border, color: display.hex, padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>Live</span>span>}
                          {currentSlot && currentSlot !== active?.slot && <span style={{ marginLeft: 8, fontSize: 12, background: "#fef3c7", color: "#92400e", padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>Manual override</span>span>}
                        </div>div>
                </div>div>
          </div>div>
        );
...
function StatCard({ label, value, accent }) {
    return (
          <div style={{ background: accent || "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px", minWidth: 0 }}>
                <div style={{ fontSize: 11, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>{label}</div>div>
                <div style={{ fontSize: 28, fontWeight: 600, color: "var(--text)", fontVariantNumeric: "tabular-nums" }}>{value}</div>div>
          </div>div>
        );
}
          ...
          <div style={{ background: rowBg, border: `1px solid ${rowBorder}`, borderRadius: 10, padding: "12px 16px", transition: "all 0.15s ease" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                    {guest.fullName}
                                    {guest.isWalkIn && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 600, background: "#fef9c3", color: "#854d0e", padding: "2px 7px", borderRadius: 20, border: "1px solid #fde68a" }}>Walk-in</span>span>}
                                  </div>div>
                                  <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 2, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                                    {guest.mlo && <span>MLO: {guest.mlo}</span>span>}
                                    {guest.slot && <span>Slot: {guest.slot}</span>span>}
                                    {sticker && (
                        ...
                        <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 9px", borderRadius: 6, background: correctSlot ? "var(--success-border)" : "var(--warn-border)", color: correctSlot ? "var(--success-text)" : "var(--warn-text)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                          ...
                          {guest.checkedIn && (
                        <button onClick={() => onUndo(guest.id)} style={{ fontSize: 12, padding: "5px 10px", borderRadius: 7, border: "1px solid var(--border)", background: "transparent", color: "var(--muted)", cursor: "pointer" }}>Undo</button>button>
                                  )}
                                  <button
            ...
            {guest.guestName && (
                    <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--border)", fontSize: 13, color: "var(--muted)", paddingLeft: 50 }}>
                              + Guest: <span style={{ color: "var(--text)", fontWeight: 500 }}>{guest.guestName}</span>span>
                    </div>div>
                )}
          </div>div>
        );
          ...
          <div style={{ "--text": "#0f0f0f", "--muted": "#6b7280", "--surface": "#ffffff", "--bg": "#f5f4f1", "--border": "#e5e7eb", "--accent": "#1a56db", "--accent-soft": "#eff6ff", "--success-bg": "#f0fdf4", "--success-border": "#86efac", "--success-text": "#15803d", "--warn-bg": "#fffbeb", "--warn-border": "#fcd34d", "--warn-text": "#92400e", fontFamily: "'Inter', system-ui, sans-serif", background: "var(--bg)", minHeight: "100vh", padding: "32px 20px" }}>
                <div style={{ maxWidth: 760, margin: "0 auto" }}>
                        <div style={{ marginBottom: 20, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
                                  <div>
                                              <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--text)", margin: 0 }}>Guest Check-In</h1>h1>
                                              <p style={{ fontSize: 14, color: "var(--muted)", marginTop: 6 }}>Antique &amp; Appraisal Fair · KQED HQ · April 9</p>p>
                                  </div>div>
                          {loaded && (
                  ...
                  {!loaded && (
                      <div
                                  ...
                                  <div style={{ fontSize: 15, color: "var(--text)", fontWeight: 500, marginBottom: 6 }}>Drop your Signal CSV here</div>div>
                                  <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 16 }}>or click to browse — reads First name, Last name, MLO, Guest name, and arrival window</div>div>
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 20 }}>
                                  <span style={{ fontSize: 13, color: "var(--muted)", marginRight: 4 }}>Active timeslot:</span>span>
                          {TIMESLOTS.map((s) => {
                  ...
                  {loaded && currentSlot && (
                      <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 14 }}>
                                  Active slot: <strong style={{ color: "var(--accent)" }}>{currentSlot}</strong>strong> — {slotGuestCount} guests expected.
                      </div>div>
                        )}
                        ...
                        filtered.length > 0 ? (
                                        ...
                                        <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontSize: 14 }}>No guests to show.</div>div>
                                      )
                      ) : (
                        <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontSize: 14 }}>Upload a CSV to get started.</div>div>
                                  )}
                        </div>div>
                </div>div>
          </div>div>
